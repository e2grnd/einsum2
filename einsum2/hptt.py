...
if __name__ == '__main__':
    print("Test")
    size=13
    a= np.random.rand(size,2*size,size,3*size,size,size,size) 
    if  a.flags['C_CONTIGUOUS']: 
        print("a is contiguous")
    axes=[6,1,2,5,0,4,3]
    start=time.time()
    b = np.transpose(a, axes=axes).copy()
    if  b.flags['C_CONTIGUOUS']: 
        print("b is contiguous")
    finish=time.time()
    npy= finish-start
    print("npy",npy)

    start=time.time()
    transpose(a, axes=axes)
    finish=time.time()
    hptt= finish-start
    print("hptt",hptt)
    print("speedup", npy/hptt)